...
{
// Este m�todo examina la pagina referenciada por P. De no //encontrarse el valor nuevo, K ser� el �ndice de la rama por //donde bajar

{
int C =  P->Claves->ObtenerApuntadorClave(1)->valorCl->getCedula();
if (Clave < C)   // recuerde que los arreglos aqu� inician en 1
  {
  	//P->K = 0;
	//P->Esta = 0;
     Encontrado=false;
...
void ArbolB::Eliminar (int C1, ApuntadorPagina Raiz){
   ...
   if (!Encontrado){
   		cout<< "El elemento no se encuentra en el �rbol"<<endl;
   }
      
   else{
   
      if (Raiz->cuenta == 0) // la ra�z ha quedado vacia
      {
      	P->cuenta;
        P= Raiz;
	...
	while (is.get(c))          // loop getting single characters
		...
		if (cont <= 4 )
			...
			if (VerificarEntero(tel_p) && VerificarEntero(cod_p))
				...
				{
				cout<<"Error en los c�digos de los proveedores.";
				return false;
				}
			}
		}
...
bool ArbolRN:: LeerCategorias(ArbolAVL &AVL) { //Leer Categor�as
